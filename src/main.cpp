...
void menu(){
    ...
    cout << "Exemplo: 03/08/2018 3 5" << endl;
}
...
void token(char *str, ItensEntrada *item){
    ...
    while(tokens != NULL){
        if(cont == 0)
            item->dia = atoi(tokens);
        else if(cont == 1)
            item->mes = atoi(tokens);
        else if (cont == 2)
            item->ano = atoi(tokens);
        else if( cont == 3)
            item->p_p = atoi(tokens);
        else if( cont == 4)
            item->p_g = atoi(tokens);
